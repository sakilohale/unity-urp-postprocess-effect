...
public class MyRBlur : ScriptableRendererFeature
{
    ...
    {   
        ...
        public string passTag="RBlurTag";
    ...
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            var RBVO = stack.GetComponent<RBVO>();       // 拿到我们的volume
            if (RBVO == null)
            {
            ...
            // 将volume组件里设置的参数传给脚本
            iterations = RBVO.iterations.value;
            blurSpread = RBVO.blurSpread.value;
            Centerx = RBVO.Centerx.value;
            Centery = RBVO.Centery.value;
            Intensity = RBVO.Intensity.value;
            // ------------------------------------------------------------------
            
            int SourceTempID = Shader.PropertyToID("_SourceTex"); 
            RenderTargetIdentifier SourceTemp = new RenderTargetIdentifier(SourceTempID); 

            passMat.SetFloat("_BlurRadius",blurSpread);
            passMat.SetFloat("_Iteration", iterations);
            passMat.SetVector("_RadialCenter",new Vector4(Centerx,Centery,0,0));
            passMat.SetFloat("_Intensity",Intensity);
            ...
            cmd.GetTemporaryRT(SourceTempID,opaquedesc,FilterMode.Bilinear);


            Blit(cmd,passSource,SourceTemp); // 存储原图
            Blit(cmd,passSource,tempory.Identifier(),passMat,0);
            Blit(cmd,tempory.Identifier(),passSource,passMat,1);

            cmd.ReleaseTemporaryRT(SourceTempID);
